...
import { Wallet, IssuedAssetId, SigningKeypair, Types } from "@stellar/typescript-wallet-sdk";
 ...
 */
export async function authenticateWithAnchor(keypair: Keypair): Promise<AuthToken> {
...
export async function initiateDeposit(
  ...
  if (!deposit.url || !deposit.id) {
    throw new Error(deposit.error || "Anchor did not return an interactive deposit URL");
  }
...
export async function initiateWithdrawal(
  ...
  if (!withdrawal.url || !withdrawal.id) {
    throw new Error(withdrawal.error || "Anchor did not return an interactive withdrawal URL");
  }
 ...
 */
export async function getTransaction(authToken: AuthToken, transactionId: string) {
  const sep24 = anchor.sep24();
 ...
 */
export async function getTransactions(authToken: AuthToken, assetCode: string = "SRT") {
  const sep24 = anchor.sep24();
